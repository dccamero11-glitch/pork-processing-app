...
def db():
    ...
    class Connection:
        ...
        def executemany(self, sql, params):
            if USE_POSTGRES:
                sql = sql.replace("?", "%s")
                with raw.cursor() as cursor:cursor.executemany(sql,params)
                return None
...
def init_db():
    with db() as conn:
        ...
        conn.execute(f"""CREATE TABLE IF NOT EXISTS orders(id {id_column},order_date DATE NOT NULL,branch TEXT NOT NULL,ordered_by TEXT NOT NULL,total_weight NUMERIC NOT NULL,note TEXT NOT NULL DEFAULT '',created_at TIMESTAMP NOT NULL)""")
        conn.execute(f"""CREATE TABLE IF NOT EXISTS order_items(id {id_column},order_id {order_id_column} NOT NULL REFERENCES orders(id) ON DELETE CASCADE,product_category TEXT NOT NULL DEFAULT 'หมู',product_name TEXT NOT NULL,quantity NUMERIC NOT NULL,unit TEXT NOT NULL,created_at TIMESTAMP NOT NULL)""")
        conn.execute(f"""CREATE TABLE IF NOT EXISTS order_requests(request_id TEXT PRIMARY KEY,order_id {order_id_column} NOT NULL REFERENCES orders(id),created_at TIMESTAMP NOT NULL)""")
    seed_users()
...
def prepare_order_items(data):
    rows=[]
    for item in data.get("items",[]):
        category=str(item.get("product_category","")).strip();name=str(item.get("product_name","")).strip()
        if category not in ORDER_CATALOG or name not in ORDER_CATALOG[category]:raise ValueError("พบหมวดหรือชื่อสินค้าที่ไม่ถูกต้อง")
        try:quantity=Decimal(str(item.get("quantity",0) or 0))
        except (InvalidOperation,ValueError):raise ValueError("น้ำหนักต้องเป็นตัวเลข")
        if not quantity.is_finite() or quantity<0:raise ValueError("น้ำหนักต้องไม่ติดลบ")
        unit=str(item.get("unit","กก.")).strip()
        if unit!="กก.":raise ValueError("หน่วยสินค้าไม่ถูกต้อง")
        if quantity>0:rows.append((category,name,quantity,unit))
    if not rows:raise ValueError("กรุณากรอกสินค้าอย่างน้อย 1 รายการที่มีน้ำหนักมากกว่า 0")
    return rows
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if clean_path == "/api/product-catalog":self.send_json({"pork":list(ORDER_PRODUCTS),"chicken":list(CHICKEN_PRODUCTS)});return
        if clean_path == "/api/orders/summary":
            from urllib.parse import parse_qs
            q=parse_qs(urlparse(self.path).query);order_date=q.get("date",[date.today().isoformat()])[0];requested=q.get("branch",["ALL" if user["role"]=="admin" else user.get("branch","")])[0]
            try:selected=effective_branch(user,requested,allow_all=True);datetime.strptime(order_date,"%Y-%m-%d")
            ...
            sql="""SELECT i.product_category,i.product_name,o.branch,SUM(i.quantity) quantity FROM orders o JOIN order_items i ON i.order_id=o.id WHERE o.order_date=?""";params=[order_date]
            if selected!="ALL":sql+=" AND o.branch=?";params.append(selected)
            sql+=" GROUP BY i.product_category,i.product_name,o.branch ORDER BY i.product_category,i.product_name,o.branch"
            with db() as conn:rows=[dict(row) for row in conn.execute(sql,params)]
            for row in rows:row["quantity"]=float(row["quantity"])
            self.send_json(rows);return
        if clean_path.startswith("/api/orders/"):
            try:order_id=int(clean_path.rsplit("/",1)[1])
            except ValueError:self.send_json({"message":"รหัสคำสั่งซื้อไม่ถูกต้อง"},400);return
            with db() as conn:
                order=conn.execute("SELECT * FROM orders WHERE id=?",(order_id,)).fetchone()
                if not order:self.send_json({"message":"ไม่พบคำสั่งซื้อ"},404);return
                try:effective_branch(user,order["branch"])
                except PermissionError as exc:self.send_json({"message":str(exc)},403);return
                items=[dict(row) for row in conn.execute("SELECT product_category,product_name,quantity,unit FROM order_items WHERE order_id=? ORDER BY id",(order_id,))]
            result=dict(order);result["order_date"]=str(result["order_date"]);result["created_at"]=str(result["created_at"]);result["total_weight"]=float(result["total_weight"])
            for item in items:item["quantity"]=float(item["quantity"])
            result["items"]=items;self.send_json(result);return
        if clean_path == "/api/orders":
            from urllib.parse import parse_qs
            q=parse_qs(urlparse(self.path).query);requested=q.get("branch",["ALL" if user["role"]=="admin" else user.get("branch","")])[0];order_date=q.get("date",[""])[0]
            try:selected=effective_branch(user,requested,allow_all=True)
            except PermissionError as exc:self.send_json({"message":str(exc)},403);return
            sql="""SELECT o.id,o.order_date,o.branch,o.ordered_by,o.total_weight,o.note,o.created_at,COUNT(i.id) item_count FROM orders o LEFT JOIN order_items i ON i.order_id=o.id WHERE 1=1""";params=[]
            if selected!="ALL":sql+=" AND o.branch=?";params.append(selected)
            ...
            with db() as conn:rows=[dict(row) for row in conn.execute(sql,params)]
            for row in rows:row["order_date"]=str(row["order_date"]);row["created_at"]=str(row["created_at"]);row["total_weight"]=float(row["total_weight"]);row["item_count"]=int(row["item_count"])
            self.send_json(rows);return
    ...
    def do_POST(self):
        try:
            ...
            if self.path == "/api/orders":
                branch=effective_branch(user,data.get("branch"));items=prepare_order_items(data);request_id=str(data.get("request_id","")).strip();note=str(data.get("note","")).strip()
                if not request_id or len(request_id)>100:raise ValueError("ไม่พบรหัสคำขอบันทึก")
                total=sum((item[2] for item in items),Decimal("0"));now=datetime.now().isoformat(timespec="seconds");stored=total if USE_POSTGRES else float(total)
                with db() as conn:
                    previous=conn.execute("SELECT order_id FROM order_requests WHERE request_id=?",(request_id,)).fetchone()
                    if previous:order_id=previous["order_id"];duplicate=True
                    else:
                        order_id=conn.execute("INSERT INTO orders(order_date,branch,ordered_by,total_weight,note,created_at) VALUES(?,?,?,?,?,?) RETURNING id",(date.today().isoformat(),branch,user["username"],stored,note,now)).fetchone()["id"]
                        conn.executemany("INSERT INTO order_items(order_id,product_category,product_name,quantity,unit,created_at) VALUES(?,?,?,?,?,?)",[(order_id,category,name,quantity if USE_POSTGRES else float(quantity),unit,now) for category,name,quantity,unit in items]);conn.execute("INSERT INTO order_requests(request_id,order_id,created_at) VALUES(?,?,?)",(request_id,order_id,now));duplicate=False
                self.send_json({"success":True,"order_id":order_id,"total_weight":float(total),"saved":len(items),"duplicate":duplicate},200 if duplicate else 201);return
